...
function loadCandidates(path: string): ModelRef[] {
  ...
  return parsed.map((entry, index) => {
    ...
    if (typeof ref.provider !== "string" || !isProviderId(ref.provider)) {
      throw new Error(`${path}[${index}]: unknown provider "${String(ref.provider)}"`);
    }
...
async function runCell(candidate: ResolvedModel, scenario: Scenario): Promise<CellResult> {
...
async function main() {
  ...
  for (const candidate of resolved) {
    console.log(`\n${candidate.label}  [${candidate.spec.wireFormat}]`);
    const results: CellResult[] = [];
    for (const scenario of SCENARIOS) {
      const result = await runCell(candidate, scenario);
      results.push(result);
  ...
  const width = Math.max(...resolved.map((c) => c.label.length));
